...
export async function getCurrentUsageLimit(
  ...
  try {
    ...
    const response = await axios.post('https://cursor.com/api/dashboard/get-hard-limit', payload, {
...
export async function checkUsageBasedStatus(
  ...
  try {
    ...
    log(
      `[API] Error details: ${JSON.stringify({
  ...
  try {
    ...
    if (teamInfo.isTeamMember && teamInfo.teamId && teamInfo.userId) {
      ...
      try {
        ...
        log(
          '[API] Team spend failed, falling back to individual usage API: ' + spendError.message,
          true,
        );
...
export async function fetchTodayUsage(
  token: string,
  teamId?: number,
): Promise<{ totalCents: number; totalUSD: number }> {
  try {
    ...
    log(
      `[API] Today usage date range: ${startDate.toISOString()} (${startTimestamp}) to ${endDate.toISOString()} (${endTimestamp})`,
    );
    ...
    log(
      '[API] Today usage API error details: ' +
        JSON.stringify({
          status: error.response?.status,
          data: error.response?.data,
          message: error.message,
...
export async function fetchTokenUsageStats(
  ...
  try {
    ...
    log(
      `[API] Token usage date range: ${startOfMonth} (${startTimestamp}) to ${endDate.toISOString()} (${endTimestamp})`,
    );
    ...
    const response = await axios.post<TokenUsageResponse>(
    ...
    log(
      `[API] Token usage stats response: totalCostCents=${response.data.totalCostCents}, aggregations=${response.data.aggregations.length}`,
    );
    // 打印response.data
    ...
    log(
      '[API] Token usage API error details: ' +
        JSON.stringify({
          status: error.response?.status,
          data: error.response?.data,
          message: error.message,
